                ...
                for (int k = 0; k < patron.length; k++) {
                    for (int l = 0; l < patron.length; l++) {
                        if (matriz[i + k][j + l] != patron[k][l]){
                            aux=false;                                                        // i + k.. xq i marca el inicio de la posicion y k son los lugares que se mueve
                            break;
                    }
                }
            }
          if (aux){
              System.out.println("Se encontro el patrón dentro de la matriz principal. En la columna "+j+" y en la fila "+i);
              return;                                                                                                               // se va a romper el método para q no nos largue el error de las coincidencias
          }
        }
    }
